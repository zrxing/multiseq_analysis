...
chip.file.names=file.names[seq(2,124,2)]
dnase.file.names=file.names[seq(1,123,2)]
...
for(j in 1:length(chip.file.names)){
  ...
  eff.for=matrix(0,10,1024)
  eff.rev=matrix(0,10,1024)

  for(i in 1:10){
    ind=sample(1:dim(dprof)[1],min(round(dim(dprof)[1]/3),10000))
  ...
  eff.nl.for=matrix(0,10,1024)
  eff.nl.rev=matrix(0,10,1024)
  ...
  pdf(paste(paste0("plots/",factor.name),"multiseq","est","log.pdf",sep="_"),width=12,height=7)
  ...
  dprof.high.s1=as.matrix(dprof[ccount>bound[2],1:1024])
  dprof.med.s1=as.matrix(dprof[ccount>bound[1]&ccount<=bound[2],1:1024])
  dprof.low.s1=as.matrix(dprof[ccount<=bound[1],1:1024])

  dprof.high.s2=as.matrix(dprof[ccount>bound[2],1025:2048])
  dprof.med.s2=as.matrix(dprof[ccount>bound[1]&ccount<=bound[2],1025:2048])
  dprof.low.s2=as.matrix(dprof[ccount<=bound[1],1025:2048])
  ...
  pdf(paste(paste0("plots/",factor.name),"group","average.pdf",sep="_"))
  par(mfrow=c(2,1))
  ...
  pdf(paste(paste0("plots/",factor.name),"group","high.pdf",sep="_"),width=12,height=7)
