...
for(j in 13:length(chip.file.names)){
  ...
  if(as.numeric(file.info(file.path("data",dnase.file.names[j]))[1])>(50*1e+6)) next
  ...
  for(l in 1:16){
    ...
    pdf(paste0("results/roc/multiple_window/", factor.name, "_multiseq_est_log", "_window_", l, ".pdf"), width = 12, height = 7)
  ...
  setwd("results/roc/multiple_window")
  ...
  pdf(paste0(factor.name, "_batf_auc_multiple_window.pdf"))
  ...
  pdf(paste0(factor.name, "_corr_pearson_multiple_window.pdf"))
  ...
  pdf(paste0(factor.name, "_corr_spearman_multiple_window.pdf"))
