...
LOG_DIR="$HOME/projects/multiseq_analysis/results/ctcf_large/logs/"
if [ ! -d "${LOG_DIR}" ]; then
    mkdir ${LOG_DIR}
fi

#run multiseq on each locus in list_loci using qsub
PROCESS_NAME=`echo "dnase_ctcf_large"`
echo "/data/tools/R-3.1.1/bin/Rscript prediction_ms_ctcf_large.R" | \
