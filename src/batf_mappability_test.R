...
dprof = dprof[map.sites[, 1] == 1, ]
cinfo = cinfo[map.sites[, 1] == 1, ]
...
for(l in 1:5){
  ...
  #     centFit[[l]]$PostPro = centFit[[l]]$PostPr[!train.ind]
  centFit.pwm[[l]] <- fitCentipede(Xlist = list(DNase = as.matrix(dprof)[, c(base.ind.for[[l]], base.ind.rev[[l]])]), Y = cbind(rep(1,dim(dprof)[1]),pscore), sweeps = 1000)
  centFit.pwm[[l]]$PostPr = centFit.pwm[[l]]$PostPr[!train.ind]
}
...
for(l in 1:5){
  ...
  roc.res.cent = roc(controls = centFit.pwm[[l]]$PostPr[unbound.ind], cases = centFit.pwm[[l]]$PostPr[bound.ind])
...
for(l in 1:5){
  roc.res.cent = rocplot(centFit.pwm[[l]]$PostPr[unbound.ind], centFit.pwm[[l]]$PostPr[bound.ind])
...
for(l in 1:5){
  ...
  corr.rank.cent[l] = cor(centFit.pwm[[l]]$PostPr, log(ccount.test + 1), method = "spearman")
}
