...
LOG_DIR="$HOME/projects/multiseq_analysis/results/roc/multiple_window/logs/"
if [ ! -d "${LOG_DIR}" ]; then
    mkdir ${LOG_DIR}
fi

#run multiseq on each locus in list_loci using qsub
PROCESS_NAME=`echo "all_tf_multiple"`
echo "/data/tools/R-3.1.1/bin/Rscript all_tf_multiple_windows.R" | \
