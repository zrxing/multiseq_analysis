...
#split into regions with low or high chipseq counts, and run multiseq on each region separately
...
ind.high = ccount.train > 24
ind.low = ccount.train <= 24
...
for(i in 1:nr){
  ...
  #base.rev[i,] = est.rev$baseline.mean
  eff.for.low[i,] = est.for$effect.mean
  #eff.rev[i,] = est.rev$effect.mean
  print(i)
}
...
lambda.for.quant.sep[1, ] = exp(multiseq(as.matrix(dprof.train.temp[, 1:nbase]), lm.approx = FALSE)$baseline.mean)
for(i in 2:nq){
  dprof.train.temp = dprof.train[(lccount.train > bin.cuts[i-1]) & (lccount.train <= bin.cuts[i]), ]
  lambda.for.quant.sep[i, ] = exp(multiseq(as.matrix(dprof.train.temp[, 1:nbase]), lm.approx = FALSE)$baseline.mean)
}

##computing the likelihood
lik.for.quant.sep = matrix(0, length(ccount.test), nq)
for(i in 1:length(ccount.test)){
  loglik.ini.for = rowSums(t(apply(lambda.for.quant.sep, 1, dpois, x = as.numeric(dprof.test[i, 1:nbase]), log = TRUE)))
  loglik.ini.for = loglik.ini.for - max(loglik.ini.for)
  #  loglik.ini.rev = rowSums(t(apply(lambda.rev,1,dpois,x = as.numeric(dprof.test[i,(nbase + 1):(2 * nbase)]),log = TRUE)))
  #  loglik.ini.rev = loglik.ini.rev - max(loglik.ini.rev)
...
dprof.train.temp = dprof.train[lccount.train <= bin.cuts[1], ]
lambda.for.quant.sep.bin[1, ] = exp(multiseq(as.matrix(dprof.train.temp[, 1:nbase]), lm.approx = TRUE)$baseline.mean)
for(i in 2:nq){
  dprof.train.temp = dprof.train[(lccount.train > bin.cuts[i-1]) & (lccount.train <= bin.cuts[i]), ]
  lambda.for.quant.sep.bin[i, ] = exp(multiseq(as.matrix(dprof.train.temp[, 1:nbase]), lm.approx = TRUE)$baseline.mean)
}

##computing the likelihood
lik.for.quant.sep.bin = matrix(0, length(ccount.test), nq)
for(i in 1:length(ccount.test)){
  loglik.ini.for = rowSums(t(apply(lambda.for.quant.sep.bin, 1, dpois, x = as.numeric(dprof.test[i, 1:nbase]), log = TRUE)))
  loglik.ini.for = loglik.ini.for - max(loglik.ini.for)
  #  loglik.ini.rev = rowSums(t(apply(lambda.rev,1,dpois,x = as.numeric(dprof.test[i,(nbase + 1):(2 * nbase)]),log = TRUE)))
  #  loglik.ini.rev = loglik.ini.rev - max(loglik.ini.rev)
...
lambda.for.quant.common.base[2, ] = lambda.for.quant.common.base[1, ] + multiseq(as.matrix(dprof.train.temp[, 1:nbase]), g.temp, lm.approx = TRUE)$effect.mean
for(i in 3:nq){
  ...
  dprof.train.temp = rbind(dprof.train.temp1, dprof.train.temp2)
  g.temp = c(rep(0, dim(dprof.train.temp1)[1]), rep(1, dim(dprof.train.temp2)[1]))
  lambda.for.quant.common.base[i, ] = lambda.for.quant.common.base[i-1, ] + multiseq(as.matrix(dprof.train.temp[, 1:nbase]), g.temp, lm.approx = TRUE)$effect.mean
}
...
for(i in 1:length(ccount.test)){
  loglik.ini.for = rowSums(t(apply(lambda.for.quant.common.base, 1, dpois, x = as.numeric(dprof.test[i, 1:nbase]), log = TRUE)))
  loglik.ini.for = loglik.ini.for - max(loglik.ini.for)
  #  loglik.ini.rev = rowSums(t(apply(lambda.rev,1,dpois,x = as.numeric(dprof.test[i,(nbase + 1):(2 * nbase)]),log = TRUE)))
  #  loglik.ini.rev = loglik.ini.rev - max(loglik.ini.rev)
...
dprof.train.temp = rbind(dprof.train.temp1, dprof.train.temp2)
g.temp = c(rep(0, dim(dprof.train.temp1)[1]), rep(1, dim(dprof.train.temp2)[1]))
multiseq.out.temp = multiseq(as.matrix(dprof.train.temp[, 1:nbase]), g.temp, lm.approx = TRUE)
...
for(i in 3:nq){
  ...
  dprof.train.temp = rbind(dprof.train.temp1, dprof.train.temp2)
  g.temp = c(rep(0, dim(dprof.train.temp1)[1]), rep(1, dim(dprof.train.temp2)[1]))
  lambda.for.quant.single.base[i, ] = lambda.for.quant.single.base[i-1, ] + multiseq(as.matrix(dprof.train.temp[, 1:nbase]), g.temp, lm.approx = TRUE)$effect.mean
}
...
for(i in 1:length(ccount.test)){
  loglik.ini.for = rowSums(t(apply(lambda.for.quant.single.base, 1, dpois, x = as.numeric(dprof.test[i, 1:nbase]), log = TRUE)))
  loglik.ini.for = loglik.ini.for - max(loglik.ini.for)
  #  loglik.ini.rev = rowSums(t(apply(lambda.rev,1,dpois,x = as.numeric(dprof.test[i,(nbase + 1):(2 * nbase)]),log = TRUE)))
  #  loglik.ini.rev = loglik.ini.rev - max(loglik.ini.rev)
...
plot(ccount.post.logmean.for.quant.single.base,log(ccount.test+1), xlim = c(0, 9), ylim = c(0, 9), pch = 16, cex = 0.5)
abline(0, 1, col = 2)
...
for(i in 1:length(ccount.test)){
  loglik.ini.for = rowSums(t(apply(lambda.for.quant.sep.base, 1, dpois, x = as.numeric(dprof.test[i, 1:nbase]), log = TRUE)))
  loglik.ini.for = loglik.ini.for - max(loglik.ini.for)
  #  loglik.ini.rev = rowSums(t(apply(lambda.rev,1,dpois,x = as.numeric(dprof.test[i,(nbase + 1):(2 * nbase)]),log = TRUE)))
  #  loglik.ini.rev = loglik.ini.rev - max(loglik.ini.rev)
