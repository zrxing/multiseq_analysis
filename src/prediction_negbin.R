...
dnbinom.mod = function(mu, x, size, prob, log = FALSE) dnbinom(x = x,  size = size, prob = prob, mu = mu, log = log)
...
for(i in 1:length(ccount.test)){
  loglik.ini.for = rowSums(t(apply(lambda.for,1,dnbinom.mod,x = as.numeric(dprof.test[i,1:nbase]),size = disp,log = TRUE)))
  loglik.ini.for = loglik.ini.for - max(loglik.ini.for)
  loglik.ini.rev = rowSums(t(apply(lambda.rev,1,dnbinom.mod,x = as.numeric(dprof.test[i,(nbase + 1):(2 * nbase)]),size = disp,log = TRUE)))
